...
export const newCoupon=TryCatch(async(req,res,next)=>{
    ...
    if(!code || !amount) return next(new Errorhandler("Plz enter both coupon and amount",401))
    await Coupon.create({
        amount,code
    })
    return res.status(201).json({
...
export const discount=TryCatch(async(req,res,next)=>{
    ...
    if(!discount) return next(new Errorhandler("Invalid Coupon", 401))
...
export const couponDelete=TryCatch(async(req,res,next)=>{
    ...
    if(!couponDelete) return next(new Errorhandler("Saly Id tu thek likh", 404))
