...
export const getAdminStats = TryCatch(async (req, res, next) => {
    ...
    const [thisMonthProduct, thisMonthUser, thisMonthOrder, lastMonthProduct, lastMonthUser, lastMonthOrder, productCount, userCount, allOrder, lastSixMonthOrder, categories, femaleUserCount, latestTranscation] = await Promise.all([
...
export const lineChart = TryCatch(async (req, res, next) => {
    ...
    order.forEach((i) => {
        ...
        if (monthDiff < 12) {
            if (property) {
                discount[12 - monthDiff - 1] += i[property];
            }
            else {
